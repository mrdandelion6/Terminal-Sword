...
int main() {
    ...
    if (dup_2 == 1) {
        ...
        char buf[200]; // allocate some space for buffer
        read(soc, buf, 200);
        buf[200] = '\0'; // need to explicitly add null terminator at end to be safe. we expect server to send a null terminator anyways but yeah.
        printf("message: %s", buf);

        usleep(2000);

        write(soc, "griffin", 200);
